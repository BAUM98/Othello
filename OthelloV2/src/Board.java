/**
 * This class provides a board object, methods for checking if a move is valid and placing
 * a piece at the valid spot, checking if there are any valid moves left, printing out the the
 * game board and tallying the final score.
 * score.
...
public class Board {
	...
	private boolean checkFlipsAndFlip(int r, int c, char currentPlayerChar, boolean flipping) {
		...
		if (downIsAValidIndex || upIsAValidIndex || rightIsAValidIndex || leftIsAValidIndex || upRightIsAValidIndex || downRightIsAValidIndex
				|| downLeftIsAValidIndex || upLeftIsAValidIndex) {
			for (int i = 1; i < 9; i++) {
				switch (i) {
				...
				for (int j = directionR, k = directionC; (j < boardArray.length && j >= 0)
					...
					if (boardArray[j][k].getColor() == currentPlayerChar && counter != 1) {
						if (flipping)
							endPieceFound = true; // End piece is set to true so the next for loop knows that a valid flip
			...
			for (int j = 0; j < boardArray[i].length; j++) {
				if (boardArray[i][j].getColor() != ' ')
					...
					if (validPlacement(i, j, currentPlayerChar)) {
						if(hint) {	
						System.out.println("Hint: " + (i+1) + " " + (j+1));
							return false;
						}
						return true;
					}
